...
use super::{AdapterContext, PrivateCapabilities, Workarounds, TextureFormatCapabilities, db, ExposedAdapter, ZERO_BUFFER_SIZE, OpenDevice};
use crate::{hal::{self, InstanceError, Capabilities, Alignments}, wgt, DeviceError};
...
pub(crate) struct AdapterShared {
    ...
    pub(crate) next_shader_id: AtomicU32,
}
...
impl Adapter {
    pub(crate) unsafe fn open(
        ...
        let queue = Queue(Arc::new(QueueInner {
            shared: Arc::clone(&self),
            features,
            draw_fbo: unsafe { gl.create_framebuffer() }.map_err(|_| DeviceError::OutOfMemory)?,
            copy_fbo: unsafe { gl.create_framebuffer() }.map_err(|_| DeviceError::OutOfMemory)?,
        ...
        Ok(OpenDevice {
            queue,
            device: Device(Arc::new(DeviceInner {
                queue: queue.clone(),
                shared: Arc::clone(&self),
                main_vao,
            })),
    ...
    pub(crate) unsafe fn texture_format_capabilities(
        ...
        use TextureFormatCapabilities as Tfc;
        ...
        let half_float_renderable = private_caps_fn(
            PrivateCapabilities::COLOR_BUFFER_HALF_FLOAT,
        ...
        let float_renderable = private_caps_fn(
            PrivateCapabilities::COLOR_BUFFER_FLOAT,
            Tfc::COLOR_ATTACHMENT
                | Tfc::COLOR_ATTACHMENT_BLEND
                | sample_count
                | Tfc::MULTISAMPLE_RESOLVE,
        ...
        let texture_float_linear =
            private_caps_fn(PrivateCapabilities::TEXTURE_FLOAT_LINEAR, filterable);
    ...
    pub(crate) unsafe fn surface_capabilities(
        ...
        if surface.presentable {
            ...
            if self
                .private_caps
                .contains(PrivateCapabilities::COLOR_BUFFER_HALF_FLOAT)
            {
    ...
    pub(crate) unsafe fn get_presentation_timestamp(&self) -> wgt::PresentationTimestamp {
        wgt::PresentationTimestamp::INVALID_TIMESTAMP
    }
}

impl Adapter {
    pub(crate) unsafe fn get_buffer_sub_data(
        &self,
        gl: &glow::Context,
        target: u32,
        offset: i32,
        ...
        if self
            .private_caps
            .contains(PrivateCapabilities::GET_BUFFER_SUB_DATA)
        {
...
impl Adapter {
    ...
    fn parse_version(mut src: &str) -> Result<(u8, u8), InstanceError> {
        let webgl_sig = "WebGL ";
    ...
    pub(crate) unsafe fn expose(context: AdapterContext) -> Option<ExposedAdapter<hal::GL>> {
        let gl = context.lock();
        ...
        let mut private_caps = PrivateCapabilities::empty();
        private_caps.set(
            PrivateCapabilities::BUFFER_ALLOCATION,
            extensions.contains("GL_EXT_buffer_storage"),
        );
        private_caps.set(PrivateCapabilities::SHADER_BINDING_LAYOUT, ver >= (3, 1));
        private_caps.set(
            PrivateCapabilities::SHADER_TEXTURE_SHADOW_LOD,
            extensions.contains("GL_EXT_texture_shadow_lod"),
        );
        private_caps.set(PrivateCapabilities::MEMORY_BARRIERS, ver >= (3, 1));
        private_caps.set(PrivateCapabilities::VERTEX_BUFFER_LAYOUT, ver >= (3, 1));
        private_caps.set(
            PrivateCapabilities::INDEX_BUFFER_ROLE_CHANGE,
            !cfg!(target_arch = "wasm32"),
        );
        private_caps.set(
            PrivateCapabilities::CAN_DISABLE_DRAW_BUFFER,
            !cfg!(target_arch = "wasm32"),
        );
        private_caps.set(
            PrivateCapabilities::GET_BUFFER_SUB_DATA,
            cfg!(target_arch = "wasm32"),
        );
        ...
        private_caps.set(PrivateCapabilities::COLOR_BUFFER_FLOAT, color_buffer_float);
        private_caps.set(
            PrivateCapabilities::TEXTURE_FLOAT_LINEAR,
        ...
        let limits = wgt::Limits {
            ...
            max_bind_groups: hal::MAX_BIND_GROUPS as u32,
            ...
            max_sampled_textures_per_shader_stage: hal::MAX_TEXTURE_SLOTS as u32,
            max_samplers_per_shader_stage: hal::MAX_SAMPLERS as u32,
            ...
            max_vertex_buffers: if private_caps.contains(PrivateCapabilities::VERTEX_BUFFER_LAYOUT)
            {
            ...
            max_vertex_attributes: (unsafe { gl.get_parameter_i32(glow::MAX_VERTEX_ATTRIBS) }
                as u32)
                .min(hal::MAX_VERTEX_ATTRIBUTES as u32),
            max_vertex_buffer_array_stride: if private_caps
                .contains(PrivateCapabilities::VERTEX_BUFFER_LAYOUT)
            {
                (unsafe { gl.get_parameter_i32(glow::MAX_VERTEX_ATTRIB_STRIDE) } as u32)
            } else {
                !0
            },
            max_push_constant_size: hal::MAX_PUSH_CONSTANTS as u32 * 4,
        ...
        workarounds.set(
            Workarounds::EMULATE_BUFFER_MAP,
            cfg!(target_arch = "wasm32"),
        );

        let r = renderer.to_lowercase();
        // Check for Mesa sRGB clear bug. See
        // [`PrivateCapabilities::MESA_I915_SRGB_SHADER_CLEAR`].
        if context.is_owned()
            && r.contains("mesa")
            && r.contains("intel")
            && r.split(&[' ', '(', ')'][..])
                .any(|substr| substr.len() == 3 && substr.chars().nth(2) == Some('l'))
        {
            ...
            workarounds.set(Workarounds::MESA_I915_SRGB_SHADER_CLEAR, true);
        }
