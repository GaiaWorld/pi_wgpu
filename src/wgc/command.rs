...
use crate::{
    hal, BindGroup, Buffer, BufferAddress, BufferSize, BufferSlice, Color, ComputePipeline,
...
#[derive(Debug)]
pub struct CommandBuffer(hal::CommandBuffer);
...
#[derive(Debug)]
pub struct CommandEncoder(hal::CommandEncoder);

impl CommandEncoder {
    /// Finishes recording and returns a [`CommandBuffer`] that can be submitted for execution.
    pub fn finish(mut self) -> CommandBuffer {
        let buffer = unsafe { self.0.end_encoding().unwrap() };
    ...
    pub fn begin_render_pass<'pass>(
        ...
        unsafe {
            let r = self.0.begin_render_pass(desc);
        }
    }
...
pub type RenderBundleDescriptor<'a> = crate::wgt::RenderBundleDescriptor<Label<'a>>;
...
pub type CommandEncoderDescriptor<'a> = crate::wgt::CommandEncoderDescriptor<Label<'a>>;

pub use crate::wgt::ImageCopyBuffer as ImageCopyBufferBase;
